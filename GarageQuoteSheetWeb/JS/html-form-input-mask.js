...
Xaprb.InputMask = {
   ...
   setupElementMasks: function() {   
      if ( document.getElementsByClassName ) { // Requires the Prototype library
         var elements=document.getElementsByClassName('input_mask');//.foreach(function(item) {
         for (var i=0;i<elements.length; i++) {
    
            Event.observe(elements[i], 'keypress',Xaprb.InputMask.applyMask.bindAsEventListener(elements[i]), true);
         }
      }
   },
